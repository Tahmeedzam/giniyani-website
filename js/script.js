...
heroIcons.forEach((icon, i) => {
  ...
  setTimeout(() => {
    icon.style.transition = "transform 1s cubic-bezier(0.16,1,0.3,1), opacity 1s ease";
...
const services = [
  ...
  {
    title: "Stock Audit",
    body: "Physical and financial verification of inventory pledged against working capital limits, carried out for banks including Jammu & Kashmir Bank, to confirm stock value and condition."
  },
];
...
// ---------- Client marquee ----------
const clientNames = [
  "Liberty Oil Mills Ltd", "Amardeep Aggregate Pvt Ltd", "Hotel Lake View",
  "Navsari Valsad Jilla Poultry Farm Co-op Society", "N R Beauty World", "Carter Corporation",
  "Mandarin International", "Amardeep Saw Mill", "Sahajanand Metals", "Ajanta Wood Products",
  "Amardeep Metals", "Harikrishna Concrete", "Doshi Enterprise", "Doshi Trading Company",
  "J K Metals", "Veritas Health Care Pvt Ltd", "Sahajanand Enterprise", "Windsons Chemicals Pvt Ltd",
  "Poshak Collection", "Bharat Corporation", "A K Garments", "Sandeep Traders", "Invogue",
  "Om Sai Enterprise", "Shivam Granite", "Eagle Trading Company", "XL Alugraphics",
];

const marqueeRow = document.getElementById("marqueeRow");
function buildMarqueeChip(name) {
  const chip = document.createElement("div");
  chip.className = "flex items-center gap-2.5 px-6 md:px-7 py-4 mx-2 rounded-full bg-white border border-hairline/60 whitespace-nowrap";
...
[...clientNames, ...clientNames].forEach(name => marqueeRow.appendChild(buildMarqueeChip(name)));
